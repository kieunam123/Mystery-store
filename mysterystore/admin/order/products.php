...
//require_once('db.php');
require_once('./../../models/iproduct.php');
class Product extends DB implements Iproduct
{
    ...
    function insert($payload, $srcs, $srcOfContent)
    {
        try {
            ...
            $size = $payload['size'];
			$chip = $payload['chip'];
			$quantity_product = $payload['quantity_product'];
         /*   $chip = $payload['chip'];
            $ram = $payload['ram'];
            $card = $payload['card'];
            $drive = $payload['drive'];
            ...
            $stm = $this->db->prepare('INSERT INTO ' .
                self::tableName . '(name,price,brand_id,keyword,short_desc,status,size,quantity_product,chip,discount)
                            VALUES(:productName,:price,:brand_id,:keyword,:short_desc,:status,:size,:quantity_product,:chip,:discount)');
            $stm->execute(array(
            ...
            for ($i = 0; $i < count($srcOfContent); $i++) {
              //  $title = $payload['title' . ($i + 1)];
               // echo $title;
                ...
                $stm2 = $this->db->prepare('INSERT INTO  detail (content) VALUES (:content)');
                $stm2->execute(array(
                   // ':img' => $srcOfContent[$i],
                    //':product_id' => $max,
                    ':content'  => $content,
    ...
    function update($payload)
    {
        try {
          ...
          /*$chip = $payload['chip'];
            ...
            ,short_desc=:short_desc,status=:status,chip=:chip,quantity_product=:quantity_product,size=:size,discount=:discount WHERE id = :id');
            $stm->execute(array(
                ':productName' => $productName,
                ':price' => $price,
                ':brand_id' => $brand_id,
