...
class Order extends DB
{
    ...
    { //in admin order
        $stm = $this->db->prepare('SELECT o.id,o.time,o.status,o.notes,o.ten,o.city,o.email,o.extra_address,u.username FROM ordered o INNER JOIN user u ON u.id = o.user_id
    ...
    function insert($payload, $cart)
    {
        try {
            ...
            $extra_address = $payload['extra_address'];
            ...
            $stm = $this->db->prepare('INSERT INTO ' .
                self::tableName . '(user_id,status,notes,ten,city,email,extra_address,time)
                 VALUES(:user_id, :status, :notes, :ten, :city, :email, :extra_address, :time)');
            $stm->execute(array(
                'user_id' => $user_id,
                'status' => 0,
                'notes' => $notes,
